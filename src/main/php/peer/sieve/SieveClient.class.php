...
use security\sasl\DigestChallenge;
 ...
 */
class SieveClient extends \lang\Object implements Traceable {
  public
    $cat      = null;
    ...
    do {
      ...
      if ('OK' == substr($line, 0, 2)) {
        ...
        switch ($key) {
          ...
          default:
            throw new \lang\FormatException('Cannot parse banner message line >'.$line.'<');
        }
        continue;
      }

      throw new \lang\FormatException('Unexpected response line >'.$line.'<');
    } while (1);
  ...
  protected function _response($discard= false, $error= true) {
    ...
    do {
      ...
      if ('OK' == substr($line, 0, 2)) {
        ...
        throw new \lang\FormatException(substr($line, 3));
      } else if ('BYE' == substr($line, 0, 3)) {
        throw new \peer\SocketException(substr($line, 4));
      } else if (!$discard) {
        $lines[]= $line;
      }
  ...
  public function authenticate($method, $user, $pass, $auth= null) {
    if (!$this->hasAuthenticationMethod($method)) {
      throw new \lang\IllegalArgumentException('Authentication method '.$method.' not supported');
    }
    ...
    switch ($method) {
      ...
      default:
        throw new \lang\IllegalArgumentException('Authentication method '.$method.' not implemented');
    }
